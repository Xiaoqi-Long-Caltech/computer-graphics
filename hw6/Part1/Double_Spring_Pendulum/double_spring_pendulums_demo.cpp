...
void update_pendulums()
{
    ...
    float root1 = sqrt(m1.x * m1.x + m1.y * m1.y);
    float root2 = sqrt((m2.x - m1.x) * (m2.x - m1.x) + (m2.y - m1.y) * (m2.y - m1.y));
    float frac1 = (root1 - m1.rl) / root1;
    float frac2 = (root2 - m2.rl) / root2;
    m1.px += dt * (-m1.k * m1.x * frac1 + m2.k * (m2.x - m1.x) * frac2);
    m1.py += dt * (-m1.k * m1.y * frac1 + m2.k * (m2.y - m1.y) * frac2 + m1.m * g);
    m2.px += dt * (-m2.k * (m2.x - m1.x) * frac2);
    m2.py += dt * (-m2.k * (m2.y - m1.y) * frac2 + m2.m * g);
    m1.x += (dt * m1.px) / m1.m;
    m1.y += (dt * m1.py) / m1.m;
    m2.x += (dt * m2.px) / m1.m;
    m2.y += (dt * m2.py) / m1.m;
